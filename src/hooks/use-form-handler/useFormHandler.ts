...
import { type FieldValues, type KeepStateOptions, type Path, type PathValue, useForm } from 'react-hook-form';
...
const useFormHandler = <T extends FieldValues>(props: FormHandlerProps<T>) => {
  ...
  const setFormValue = (args: SetValueProps) => {
    formInstance.setValue(args.name as Path<T>, args.value as PathValue<T, Path<T>>, {
      shouldValidate: true,
      shouldDirty: true,
      ...args.options,
    });
  ...
  const fieldState = (name: Path<T>) => {
    const { isDirty, isTouched, invalid, error } = formInstance.getFieldState(name);
   ...
   */
  const resetForm = (values?: Partial<T>, options?: KeepStateOptions) => {
    clearErrors();

    if (values != null) {
      formInstance.reset(values as T, options);
      return;
    }
   ...
   */
  const onSubmitHandler = formInstance.handleSubmit(props.onSubmit, props.onError);
  ...
  const debugMode = (): DebugModeState => {
    const { defaultValues, currentState, isValid, errors, hasBeenUpdated } = formState();
